...
define('COOKIE_FILE_NAME', dirname(__FILE__).'/unifi.cookies');
...
class UnifiApi
{
    ...
    public function login()
    {
        ...
        if (isset($results[1])) {
            $this->set_cookies(implode(';', $results[1]));
            
            if (!empty($body)) {
                if (($code >= 200) && ($code < 400)) {
                    if (strpos($this->get_cookies(), 'unifises') !== false) {
                        $is_loggedin = true;
                    }
                }
    ...
    {
        //if (!$this->is_loggedin) return false;
        $json               = json_encode(array('name' => $group_name, 'qos_rate_max_down' => $group_dn, 'qos_rate_max_up' => $group_up));
    ...
    {
        //if (!$this->is_loggedin) return false;
        $json            = json_encode(array('radio_table' => array('radio' => $radio, 'channel' => $channel, 'ht' => $ht, 'tx_power_mode' => $tx_power_mode, 'tx_power' =>$tx_power)));
    ...
    {
        //if (!$this->is_loggedin) return false;
        $json            = array('_sort' => '-time', 'within' => $historyhours, 'type' => null, '_start' => $start, '_limit' => $limit);
        $json            = json_encode($json);
    ...
    {
        if ($this->cookies == '') {
            $this->cookies = file_get_contents(COOKIE_FILE_NAME);
        }
     ...
     * Update cookies file
     */
    private function set_cookies($data)
    {
        if ($this->cookies != $data) {
            file_put_contents(COOKIE_FILE_NAME, $data);
            $this->cookies = $data;
        }
    ...
    /**
     * Execute the cURL request and login user if required
     */
    private function exec_curl($url, $data = '')
    {
        try {
            $result = $this->exec_curl_raw($url, $data);
        } catch (LoginRequiredException $e) {
            $this->login();
            $result = $this->exec_curl_raw($url, $data, false);
        }
     ...
     */
    private function exec_curl_raw($url, $data, $raise_401 = true)
    {
        $ch = $this->get_curl_obj();
        ...
        if ($raise_401 && curl_getinfo($ch, CURLINFO_HTTP_CODE) == 401) {
